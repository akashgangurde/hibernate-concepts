...
public class EmpHome {
	...
	public Emp findById(java.lang.Integer id) {
		...
		try {
			Emp instance = (Emp) sessionFactory.getCurrentSession().get("com.Emp", id);
			if (instance == null) {
	...
	public List findByExample(Emp instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("com.Emp").add(Example.create(instance))
					.list();
